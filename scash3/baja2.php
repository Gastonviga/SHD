        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if ($DNI > 0) {
                ...
                if ($sqlExistencia->num_rows > 0) {
                    ...
                    $sqlEliminar = $conexion->prepare("DELETE FROM Alumno WHERE DNI = ?");
                    $sqlEliminar->bind_param("i", $DNI);
                    $sqlEliminar->execute();

                    // Verificar si la eliminación fue exitosa
                    if ($sqlEliminar->affected_rows > 0) {
                        echo "El alumno con DNI $DNI ha sido eliminado de la base de datos";
                    } else {
                        echo "Error al intentar eliminar al alumno. Por favor, inténtalo de nuevo.";
                    }
